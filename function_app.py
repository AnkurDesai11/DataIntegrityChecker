...
def akv_extractor(dataDict: dict) -> str:
    ...
    # Setup thread lock which can be acquired by threads while writing to global dataframe and output file
    ...
    def thread_safe_appender(vault_name, secret_list, vault_resource_group, status):
        ...
        # Acquire tread lock to write to global dataframe/output file
        thread_lock.acquire()
    ...
    def worker_thread(vault_headers):
        while not shared_queue.empty():
            try:
                # Get vault from queue without blocking
                current_vault = shared_queue.get()
                ...
                if(secret_names is None):
                    thread_safe_appender(current_vault["name"], secret_names, current_vault["id"].split('/')[4], secrets_in_current_vault)
                else:
                    thread_safe_appender(current_vault["name"], secret_names, current_vault["id"].split('/')[4], "Read Secrets Succesful for vault")
    ...
    vaults_in_subscription = [{"name": "b-prashanthpa-0360e20a94"}]

    # Load list of vaults in the subscription into thread safe queue
    [shared_queue.put(vault) for vault in vaults_in_subscription]
    number_of_vaults_in_subscription = len(vaults_in_subscription)
    vaults_in_subscription = None

    for i in range(number_of_threads):
        t = threading.Thread(target = worker_thread, args=(vault_headers,))
        t.start()
