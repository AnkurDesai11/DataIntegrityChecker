...
def akv_extractor(dataDict: dict) -> str:
    ...
    global shared_queue, number_of_vaults_in_subscription
...
def thread_safe_appender(vault_name, secret_list, status):
    global thread_lock, all_vault_secrets, output_data_columns
...
def worker_thread(vault_headers):
    ...
    while not shared_queue.empty():
        try:
            ...
            vault_url = "https://{}.vault.azure.net/secrets?maxresults={}&api-version=7.4".format(current_vault["name"], 25)
...
def save_to_csv(df_to_append):
    try:
        ...
        blob_service_client = BlobServiceClient.from_connection_string("AzureWebJobsStorage")
        ...
        output_df = existing_df.append(df_to_append, ignore_index=True)
